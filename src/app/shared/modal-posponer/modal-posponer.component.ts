...
@Component({
  selector: 'app-modal-posponer',
  templateUrl: './modal-posponer.component.html',
  styleUrls: ['./modal-posponer.component.scss'],
})
export class ModalPosponerComponent implements OnInit {
    ...
    this.publicationService
      ...
      .subscribe(
        (data) => {
          ...
          if (data.message) {
            this.toastHelper.presentToastError(data.message || 'No hay fechas disponibles para posponer');
            this.closeModel();
          }
        },
        (error) => {
    ...
    this.publicationService
      ...
      .subscribe(
        async (data) => {
          ...
          if (data.message) {
            this.toastHelper.presentToastError(data.message || 'No hay horas disponibles para posponer, intente con otra fecha');
          }
        },
        (error) => {
          this.toastHelper.presentToastError(error.error.message);
    ...
    this.publicationService.postReserve(sendForm, this.id)
      ...
      .subscribe((data) => {
        this.alertHelper.alertSingleOption(
          true,
          data.message || "Mentoría pospuesta correctamente",
